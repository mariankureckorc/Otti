/* Reminders + Community + Articles */

// ─────────────────────────────────────────────────────────────
// 13 — Notification permission request
// ─────────────────────────────────────────────────────────────
function ScreenNotifPermission() {
  return (
    <Phone bg={OTTI.cream}>
      ...
      <div style={{ position: 'absolute', bottom: 50, left: 28, right: 28 }}>
        <Btn>Turn on reminders</Btn>
        <div style={{ marginTop: 10 }}>
          <Btn kind="ghost">Not right now</Btn>
        </div>
      </div>
    </Phone>
  );
...
// ─────────────────────────────────────────────────────────────
// 14 — Reminder settings
// ─────────────────────────────────────────────────────────────
function ScreenReminderSettings() {
  return (
    <Phone bg={OTTI.cream}>
      ...
      <div style={{ padding: '0 20px' }}>
        ...
        <div style={{ marginTop: 8, background: '#fff', borderRadius: 18, border: `1px solid ${OTTI.lineSolid}`, overflow: 'hidden' }}>
          {[
            { l: 'Only if Mia is behind goal', sel: true, d: 'A gentle check-in around 4pm' },
            { l: 'Twice a day', sel: false, d: 'Morning and mid-afternoon' },
            { l: 'Every 2 hours during day', sel: false, d: 'For those big catch-up days' },
          ].map((opt, i, arr) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', padding: '14px 16px', borderBottom: i < arr.length - 1 ? `1px solid ${OTTI.lineSolid}` : 'none', gap: 14 }}>
              <div style={{
                width: 22, height: 22, borderRadius: 11,
                border: opt.sel ? 'none' : `1.5px solid ${OTTI.ink4}`,
                background: opt.sel ? OTTI.navy : 'transparent',
...
function Toggle({ on = false }) {
  return (
    <div style={{
      ...
      background: on ? OTTI.green : OTTI.lineSolid,
      position: 'relative', flexShrink: 0,
    }}>
      <div style={{
        position: 'absolute', top: 2, left: on ? 22 : 2, width: 26, height: 26, borderRadius: 13,
    ...
    <Phone bg={OTTI.cream}>
      <div style={{ paddingTop: 60, padding: '60px 20px 0' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button style={{ height: 38, padding: '0 14px', borderRadius: 19, background: OTTI.navy, color: '#fff', border: 'none', fontWeight: 700, fontSize: 13, fontFamily: SANS, display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{ marginTop: 18, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          {categories.map(c => (
            <div key={c.l} style={{ background: c.color, borderRadius: 16, padding: '12px 14px' }}>
        ...
        <div style={{ marginTop: 22, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 4px' }}>
          ...
          <div style={{ fontSize: 12, color: OTTI.navy, fontWeight: 600 }}>See all</div>
        </div>

        <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 8 }}>
          {threads.map((t, i) => (
            <div key={i} style={{ background: '#fff', borderRadius: 16, padding: '14px 14px', border: `1px solid ${OTTI.lineSolid}`, display: 'flex', gap: 12 }}>
  ...
  return (
    <Phone bg={OTTI.cream}>
      ...
      <div style={{ padding: '0 20px', paddingBottom: 80 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 22, padding: '18px 18px', border: `1px solid ${OTTI.lineSolid}` }}>
          ...
          <div style={{ marginTop: 14, display: 'flex', gap: 18, fontSize: 13, fontWeight: 600 }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 6, color: OTTI.coral }}>{Icon.heart(OTTI.coral, 18, true)} 52</span>
            ...
            <div key={i} style={{ background: '#fff', borderRadius: 16, padding: '14px 14px', border: r.official ? `1px solid ${OTTI.navy}` : `1px solid ${OTTI.lineSolid}`, display: 'flex', gap: 10 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ marginTop: 8, display: 'flex', gap: 14, fontSize: 12, color: OTTI.ink3, fontWeight: 600 }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>{Icon.heart(OTTI.ink3, 13)} {r.like}</span>
                  <span>Reply</span>
                </div>
              </div>
            </div>
      ...
      <div style={{
        ...
        borderTop: `1px solid ${OTTI.line}`, display: 'flex', alignItems: 'center', gap: 10,
      }}>
        <div style={{ flex: 1, height: 44, borderRadius: 22, background: '#fff', border: `1px solid ${OTTI.lineSolid}`, display: 'flex', alignItems: 'center', padding: '0 16px', fontSize: 14, color: OTTI.ink3 }}>
          Write a reply…
        </div>
        <div style={{ width: 44, height: 44, borderRadius: 22, background: OTTI.navy, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
// ─────────────────────────────────────────────────────────────
// 17 — New post composer
// ─────────────────────────────────────────────────────────────
function ScreenNewPost() {
  return (
    <Phone bg={OTTI.cream}>
      <Header title="New post" close back={false} trailing={<div style={{ fontSize: 14, fontWeight: 700, color: OTTI.navy }}>Post</div>} />
      <div style={{ padding: '0 20px' }}>
        {/* Category */}
        <div style={{
        ...
        <div style={{ position: 'absolute', bottom: 76, left: 20, right: 20, background: OTTI.navyTint, borderRadius: 14, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ fontSize: 12, fontWeight: 600, color: OTTI.navy }}>Change</div>
        </div>

        {/* Tools */}
        <div style={{ position: 'absolute', bottom: 30, left: 20, right: 20, display: 'flex', gap: 8 }}>
          {['Aa','📎','🙂'].map((t, i) => (
            <div key={i} style={{ width: 40, height: 40, borderRadius: 12, background: '#fff', border: `1px solid ${OTTI.lineSolid}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, color: OTTI.ink2, fontWeight: 700 }}>{t}</div>
          ))}
          <div style={{ flex: 1 }} />
      ...
      <div style={{ paddingTop: 60, padding: '60px 20px 0' }}>
        ...
        <div style={{ marginTop: 18, display: 'flex', gap: 6, overflowX: 'hidden' }}>
          {['All', 'Early years', 'Devices', 'Speech', 'School', 'Family'].map((c, i) => (
            <div key={c} style={{
              height: 32, padding: '0 14px', borderRadius: 16, flexShrink: 0,
        ...
        {/* Featured */}
        <div style={{
          marginTop: 18, height: 200, borderRadius: 24, position: 'relative', overflow: 'hidden',
        ...
        <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 8 }}>
          {articles.map((a, i) => (
            <div key={i} style={{ background: '#fff', borderRadius: 16, padding: 12, border: `1px solid ${OTTI.lineSolid}`, display: 'flex', gap: 12, alignItems: 'center' }}>
  ...
  return (
    <Phone bg={'#fff'}>
      ...
      <div style={{
        ...
        {/* status bar overlay protection: keep dark area at top */}
        ...
        <div style={{ position: 'absolute', top: 56, left: 20, right: 20, display: 'flex', justifyContent: 'space-between' }}>
          <div style={{ width: 40, height: 40, borderRadius: 20, background: 'rgba(255,255,255,0.18)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{Icon.back('#fff', 20)}</div>
          <div style={{ width: 40, height: 40, borderRadius: 20, background: 'rgba(255,255,255,0.18)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {Icon.heart('#fff', 20)}
          </div>
        </div>
      ...
      <div style={{
        ...
        <button style={{ height: 44, padding: '0 22px', borderRadius: 22, background: OTTI.navy, color: '#fff', border: 'none', fontWeight: 700, fontFamily: SANS, fontSize: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
...
Object.assign(window, { ScreenNotifPermission, ScreenReminderSettings, ScreenForum, ScreenThread, ScreenNewPost, ScreenArticleList, ScreenArticleDetail, Toggle });
