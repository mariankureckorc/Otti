...
// — Post-signup: email-verification screens ————————————————————

// Step 1/2 of post-signup: waiting for the user to click the link in their
// inbox. Auto-progresses to emailVerified after 8s for the hands-free demo.
function ScreenVerifyWaiting({ nav }) {
  const { draft } = useSignupDraft();
  const email = (draft.email && draft.email.trim()) || 'your email';
  ...
  React.useEffect(() => {
    const t = setTimeout(() => nav('emailVerified'), 8000);
    return () => clearTimeout(t);
  }, [nav]);

  const textLink = {
    background: 'transparent', border: 'none', cursor: 'pointer',
    color: OTTI.navy, fontWeight: 700, fontFamily: SANS,
  ...
  return (
    <Phone bg={OTTI.cream}>
      ...
      <div style={{
        position: 'absolute', inset: 0, padding: '70px 28px 100px',
        ...
        <div style={{
          marginTop: 24, fontSize: 24, fontWeight: 800, color: OTTI.navyDeep,
          letterSpacing: -0.5, lineHeight: 1.2,
        }}>
          Verify your email
        </div>

        <div style={{ marginTop: 10, fontSize: 14, color: OTTI.ink2, lineHeight: 1.5, maxWidth: 300 }}>
          We've sent a verification link to your email. Open your inbox and click the link to activate your account.
        </div>

        {/* Email address shown to the user — derived from Step 1 (or the
            OAuth provider's pre-fill on Step 1). */}
        <div style={{
          marginTop: 12, padding: '8px 14px',
          background: '#fff', borderRadius: 12,
          border: `1px solid ${OTTI.lineSolid}`,
          fontSize: 14, fontWeight: 700, color: OTTI.navyDeep,
          fontVariantNumeric: 'tabular-nums', maxWidth: '100%',
        ...
        <div style={{ marginTop: 18, display: 'flex', gap: 6 }}>
          {[0, 1, 2].map(i => (
            <div key={i} style={{
              width: 8, height: 8, borderRadius: 4, background: OTTI.navy,
        ...
        <div style={{ marginTop: 22, fontSize: 13, color: OTTI.ink3, lineHeight: 1.7 }}>
          <div>
      ...
      {/* Demo helper — anchored at bottom */}
      <div style={{
        position: 'absolute', bottom: 28, left: 0, right: 0, textAlign: 'center',
      }}>
        <button
          onClick={() => nav('emailVerified')}
          style={{
            background: 'transparent', border: 'none', cursor: 'pointer',
            fontFamily: SANS, fontSize: 13, fontWeight: 700, color: OTTI.navy,
            padding: '8px 16px', textDecoration: 'underline',
          }}
        >
          Simulate email click (demo only)
        </button>
      </div>
  ...
  return (
    <Phone bg={OTTI.cream}>
      <div style={{
        ...
        {/* Mascot with celebration halo + check badge */}
        <div style={{ position: 'relative', display: 'inline-block' }}>
          <div style={{
            position: 'absolute', inset: -28, borderRadius: '50%', background: OTTI.greenSoft,
          }} />
          ...
          {/* Green check badge — small celebration accent */}
          <div style={{
            position: 'absolute', bottom: -4, right: -8,
            width: 48, height: 48, borderRadius: 24,
            background: OTTI.green,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            border: '3px solid #fff',
            boxShadow: '0 4px 14px rgba(141, 173, 42, 0.4)',
          }}>
            {Icon.check(OTTI.navyDeep, 24)}
          </div>
        </div>

        <div style={{
          marginTop: 28, fontSize: 26, fontWeight: 800, color: OTTI.navyDeep,
          letterSpacing: -0.5, lineHeight: 1.2,
        }}>
          Email verified!
        </div>

        <div style={{ marginTop: 12, fontSize: 15, color: OTTI.ink2, lineHeight: 1.55, maxWidth: 280 }}>
          You're all set. Welcome to Otti.
        </div>
      </div>

      <div style={{ position: 'absolute', bottom: 36, left: 24, right: 24 }}>
        <Btn onClick={finish}>Continue to Otti</Btn>
      </div>
    </Phone>
  );
}
...
Object.assign(window, {
  ScreenSignupStep1, ScreenSignupStep2,
  ScreenVerifyWaiting, ScreenEmailVerified,
});
